...
def get_raw_image(uav_name, pos):
    response = client.simGetImages([airsim.ImageRequest(pos, airsim.ImageType.Scene, False, False)],
                                        vehicle_name=uav_name)
    if response[0] is None:
        print("received empty data: data interface at " + uav_name)
    else:
        # get numpy array
...
client.enableApiControl(True, "cf101")
num = 0
client.moveToPositionAsync(0,0,-5,5,vehicle_name="cf101").join()
plt.ion()
...
while True:
    ...
    t1 = time_sync()
    img_rgb = get_raw_image("cf101", 0)

    # airsim.write_png(os.path.normpath('mypic' + '.jpg'), img_rgb)
    # path = 'mypic.jpg'
    # img = cv2.imread(path)
